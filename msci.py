...
url = 'https://www.stockq.org/index/MS001.php'
driver = webdriver.Firefox()                                                        #create webbrowser
driver.get(url)

if os.path.isfile('MSCI.csv') != 1:                                                 #if there is no such file ,download all datas that website have
    try:
...
for i in range(1,len(table)):
    ...
    tlist.append([date,price,MA20,MA60,MA120])
df = pd.DataFrame(tlist,columns=th).rename(columns={'Time':'Date'})
df['Date'] = pd.to_datetime(df['Date'], format='mixed')
if os.path.isfile('MSCI.csv'):                                                       #check if this file exist
    data = pd.read_csv('MSCI.csv')
    ...
    df.to_csv('MSCI.csv')
print(pd.read_csv('MSCI.csv'))                                                       #check if every data update successfully
